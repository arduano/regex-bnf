    ...
    fn as_parse_quote(&self, input: &Ident) -> proc_macro2::TokenStream {
        let inner = match self {
            Self::Term { name, modifier } => match modifier {
                ...
                Some(BnfTermArgModifier::Vec(len_kind)) => {
                    let length = quote! { length };

                    let length_check = match len_kind {
                        VecRepeatKind::ZeroOrMore => quote! {},
                        VecRepeatKind::OneOrMore => quote! {
                            if #length == 0 {
                                return Err(ParseError::new(concat!(stringify!(#name), " expected at least one element"), #input.pos));
                            }
                        },
                    };
        ...
        if input.peek(Token![<]) {
            ...
            if input.peek(Bracket) {
                ...
                if input.peek(Token![^]) {
                    ...
                    let _: Token![+] = input.parse()?;
                    modifier = Some(BnfTermArgModifier::Vec(VecRepeatKind::OneOrMore))
                } else if input.peek(Token![*]) {
                    let _: Token![*] = input.parse()?;
                    modifier = Some(BnfTermArgModifier::Vec(VecRepeatKind::ZeroOrMore))
                } else {
                    return Err(input.error("Expected +, *, or ^ after [token]"));
                }
